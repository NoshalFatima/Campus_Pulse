...
class _FacultyProfileScreenState extends State<FacultyProfileScreen>
  ...
  late TextEditingController _majorSubjectCtrl;
  ...
  void initState() {
    ...
    _majorSubjectCtrl = TextEditingController();
    ...
    try {
      ...
      if (doc.exists) {
        ...
        setState(() {
          ...
          _contactCtrl.text      = data['contactNumber']?.toString() ?? '';
          _qualificationCtrl.text = data['qualification']?.toString() ?? '';
          _experienceCtrl.text   = data['experience']?.toString() ?? '';
          _gradeCtrl.text        = data['grade']?.toString() ?? '';
          _majorSubjectCtrl.text = data['majorSubject']?.toString() ?? '';
          _isLoading = false;
        });
        _fadeCtrl.forward();
      }
...
String uploadPreset = dotenv.env['CLOUDINARY_UPLOAD_PRESET'] ?? '';
    ...
    try {
      ...
      req.files.add(http.MultipartFile.fromBytes(
        'file', bytes,
        filename: 'faculty_${DateTime.now().millisecondsSinceEpoch}.jpg',
  ...
  void _cancelEdit() {
    ...
    _contactCtrl.text       = _userData['contactNumber']?.toString() ?? '';
    _qualificationCtrl.text = _userData['qualification']?.toString() ?? '';
    _experienceCtrl.text    = _userData['experience']?.toString() ?? '';
    _gradeCtrl.text         = _userData['grade']?.toString() ?? '';
    _majorSubjectCtrl.text  = _userData['majorSubject']?.toString() ?? '';
    setState(() {
      _newProfileBytes = null;
      _isEditing       = false;
    });
  ...
  // ════════════════════════════════════════════════════
  //  BUILD — same announcement-screen wrapper
  // ════════════════════════════════════════════════════
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      // ✅ Transparent — dashboard header shows fully (same as announcement)
                        ...
                        Padding(
                          ...
                          child: Column(children: [
                            ...
                            _buildSection(
                              ...
                              children: [
                                // Locked
                                _buildInfoRow(Icons.numbers, "Employee ID",
                                    _str('employeeId'), locked: _isEditing),
                                _buildInfoRow(Icons.account_balance_outlined,
                                    "Department", _str('dept'),
                                    locked: _isEditing),
                                _buildInfoRow(Icons.workspace_premium_outlined,
                                    "Designation", _str('designation'),
                                    locked: _isEditing),
                                _buildInfoRow(Icons.calendar_today_outlined,
                                    "Joining Date", _str('joiningDate'),
                                    locked: _isEditing),
                                // Editable
                                if (_isEditing) ...[
                                  _buildEditRow("Qualification", _qualificationCtrl),
                                  _buildEditRow("Experience (years)", _experienceCtrl,
                                      isNum: true),
                                  _buildEditRow("Grade", _gradeCtrl),
                                  _buildEditRow("Major Subject", _majorSubjectCtrl),
                                ] else ...[
                                  _buildInfoRow(Icons.school,
                                      "Qualification", _str('qualification')),
                                  _buildInfoRow(Icons.work_outline,
                                      "Experience", _str('experience')),
                                  _buildInfoRow(Icons.grade_outlined,
                                      "Grade", _str('grade')),
                                  _buildInfoRow(Icons.book_outlined,
                                      "Major Subject", _str('majorSubject')),
                                ],
                              ],
                            ),
                            const SizedBox(height: 14),
  ...
  // ════════════════════════════════════════════════════
  //  HERO CARD — HeaderCurvedPainter + overlapping pic
  // ════════════════════════════════════════════════════
    ...
    child: Stack(
      ...
      children: [
        ...
        Positioned(
          ...
          child: SizedBox(
            height: 170,
            // ✅ Yahan ClipRRect lagayein taake CustomPainter corners se bahar na nikle
            child: ClipRRect(
              borderRadius: BorderRadius.circular(20), // Jitna radius aapko chahiye
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          _str('designation', 'Faculty Member'),
                          style: const TextStyle(
                        ...
                        Text(
                          _str('dept', ''),
                          style: TextStyle(
                              color: _gold.withOpacity(0.9), fontSize: 12),
                        ),
                      ],
                    ),
                  ),
  ...
  Widget _buildLockedBanner() {
    return Container(
      ...
      child: const Row(children: [
        ...
        Expanded(
            child: Text(
                "Employee ID, CNIC, Dept., Designation & Joining Date cannot be changed.",
  ...
  Widget _buildEditRow(String label, TextEditingController ctrl,
      {bool isPhone = false, bool isNum = false}) {
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
      child: TextField(
        controller: ctrl,
        keyboardType: (isPhone || isNum)
...
// ════════════════════════════════════════════════════
//  HeaderCurvedPainter — exact as provided
// ════════════════════════════════════════════════════
class HeaderCurvedPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Layer 3: Alpha Layer (#A11321, 0.3 opacity)
